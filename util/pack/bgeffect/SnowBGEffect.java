...
public class SnowBGEffect extends BackgroundEffect{
    ...
    public void postDraw(FakeGraphics g, P rect, float siz, float midH) {
        ...
        g.setComposite(FakeGraphics.TRANS, 50, 0);

        for(int i = 0; i < snowPosition.size(); i++) {
            g.fillOval(convertP(snowPosition.get(i).x, siz) + (int) rect.x,
                    (int) (snowPosition.get(i).y * siz - rect.y + midH * siz),
                    sw * siz,
                    sh * siz);
        }

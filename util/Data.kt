...
object Data {
        ...
        abstract class ProcItem : Cloneable {
            fun clear(): ProcItem {
                try {
                    ...
                    for (f in fs)
                        when {
                            f.type == Int::class.javaPrimitiveType -> f[this] = 0
                            Data.Proc.IntType::class.java.isAssignableFrom(f.type) -> f[this] = f.type.newInstance()
                            f.type == PackData.Identifier::class.java -> f[this] = null
                            else -> throw Exception("unknown field " + f.type + " " + f.name)
                        }
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            ...
            public override fun clone(): ProcItem {
                ...
                for (f in fs)
                    if (Data.Proc.IntType::class.java.isAssignableFrom(f.type))
                        f[ans] = (f[this] as Data.Proc.IntType).clone()
                    else if (f.type == PackData.Identifier::class.java && f[this] != null)
                        f[ans] = (f[this] as PackData.Identifier<*>).clone()
                return ans
    ...
    fun restrict(str: String): String {
        return if (str.length < restrict_name) str else str.substring(0, restrict_name)
    }

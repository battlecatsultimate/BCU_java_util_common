...
public class AttackSimple extends AttackAb {
	...
	public void capture() {
		...
		if(attacker != null) {
			if(attacker.dire == -1 && attacker.pos <= model.b.getBase(attacker.dire).pos && isLongAtk && !le.contains(model.b.getBase(attacker.dire))) {
				le.add(model.b.getBase(attacker.dire));
			} else if (attacker.dire == 1 && attacker.pos >= model.b.getBase(attacker.dire).pos && isLongAtk && !le.contains(model.b.getBase(attacker.dire))) {
				le.add(model.b.getBase(attacker.dire));
			}
		}
